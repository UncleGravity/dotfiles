...
for command in age-keygen chmod cp git install jq mktemp nix rm sops ssh-keygen stat; do
...
echo "Staged and verified the Clan machine identity for '$host' under '$root'; no /etc/ssh host identity was staged."
